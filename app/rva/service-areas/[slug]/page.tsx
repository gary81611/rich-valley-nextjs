...
/** When service-area slug differs from `locations.slug` (e.g. snowmass-village vs snowmass). */
const SERVICE_AREA_SLUG_TO_LOCATION_SLUG: Record<string, string> = {
...
async function getLocationForServiceAreaSlug(pathSlug: string) {
  const supabase = await createServerSupabaseClient()
  const locationSlug = SERVICE_AREA_SLUG_TO_LOCATION_SLUG[pathSlug] ?? pathSlug
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (area) {
    const desc = (area.description || area.long_description || '')
      ...
      .slice(0, 160)
    return {
      title: `${area.name} | Guided Adventures | Rich Valley Adventures`,
  ...
  const loc = await getLocationForServiceAreaSlug(slug)
  if (loc) {
    return {
...
export default async function RvaServiceAreaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
      ...
      <div className="min-h-screen bg-rva-cream font-inter">
        ...
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            ...
            <Link
              href="/locations"
              className="inline-flex items-center justify-center border border-rva-forest text-rva-forest font-semibold px-6 py-3 rounded-full hover:bg-rva-cream-dark/50 transition-colors"
            >
              Location guides
            </Link>
          </div>
  ...
  if (loc) {
    return (
      <LocationDetailView
        loc={loc}
        backLink={{ href: '/service-areas', label: '← Service areas' }}
      />
    )
  }
